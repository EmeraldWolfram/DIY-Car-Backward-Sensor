...
int checkIfCompatible(){
  
  //list of PIC18F2XXX/4XXX family IDs
  //#define numIDs 12
  //byte IDs[numIDs] = {0x21 ,0x11 ,0x24 ,0x12 ,0x2A ,0x1A ,0x0C ,0x2A ,0x0E ,0x27 ,0x10 ,0x24};
  //assume same ID, works the same.
  #define numIDs 3
  byte IDs[numIDs] = {0x11, 0x12, 0x10};
  /*
  Confirmed to work:
  PIC18F2420
  PIC18F2550
  ...
  */
  ...
  for(int i = 0; i < numIDs; i++){ //keep trying to match IDs
  if( readFlash(0x3f,0xff,0xff) == IDs[i] ){  //read from DIVID2 register
    return 1;
  }
  }
